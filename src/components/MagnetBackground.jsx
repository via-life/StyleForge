...
export default function MagnetBackground({ performanceTier = 'desktop' }) {
  const isMobile = performanceTier === 'mobile';
  const isLowEndMobile = performanceTier === 'low-end-mobile';

  return (
    <div
      className={`magnet-background ${isMobile ? 'is-mobile' : ''} ${
        isLowEndMobile ? 'is-low-end-mobile' : ''
      }`}
      aria-hidden="true"
    >
      <div className="magnet-background__pattern" />
      <div className="magnet-background__glow magnet-background__glow--mint" />
      {!isLowEndMobile ? <div className="magnet-background__glow magnet-background__glow--peach" /> : null}
      ...
      {isMobile ? (
        <MagnetLines
          rows={10}
          columns={14}
          containerSize="min(96vw, 44rem)"
          lineColor="rgba(29, 52, 68, 0.08)"
          lineWidth="2px"
          lineHeight="22px"
          baseAngle={-10}
          interactive={false}
          className="magnet-background__field magnet-background__field--mobile"
        />
      ) : null}
    </div>
  );
